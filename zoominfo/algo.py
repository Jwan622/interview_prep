...
def some_function_recursive(account):
    ...
    # Correct the final values based on the rule that None values point to themselves
    for key in resolved:
        if account[key] is None:
            resolved[key] = key
    return resolved
...
account = {'a':'b',
           ...
           'h':'b',
           'i':'d'}
assert some_function_recursive(account) == {'a': 'd', 'c': 'd', 'b': 'd', 'd': 'd', 'e': 'f', 'f': 'f', 'g': 'g', 'h': 'd', 'i': 'd'}

# account1 = {'a':'b',
#            'b': None}
# assert some_function_recursive(account1) == {'a': 'b', 'b': 'b'}
#
# account2 = {'a':'b',
#            'b': 'd',
#             'd': None
#             }
# assert some_function_recursive(account2) == {'a': 'd', 'b': 'd', 'd': 'd'}
