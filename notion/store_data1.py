...
# top_revenue_per_store = sorted_df.groupby('Store').head(3)
# print('top revenue per store: \n', top_revenue_per_store)
grouped_df = df_over_2000.groupby(['Store', 'Product']).agg({'Total_Revenue': 'sum'})
sorted_df = grouped_df.sort_values(by=['Store', 'Total_Revenue'], ascending=[True, False])
print("sorted revenue df: \n", sorted_df)
top_products_within_stores = sorted_df.groupby("Store").head(3)
print("top_products_within_stores \n", top_products_within_stores)
